# The `dryrun:` execution extension (design spec, v0.1)
...
`dryrun:` is our dialect for exactly that gap — the browser runtime's
...
2. Stripping every `dryrun:` element leaves a complete, valid, importable
...
Namespace: `xmlns:dryrun="https://dryrun.dev/schema/1.0"` (name/URI is an
open question in the brief).

## Vocabulary

### `dryrun:mock` — browser stand-in for an unbound task
...
### `dryrun:binding` — declared real-world implementation intent
Optional, same elements as `dryrun:mock`. *Descriptive, not executable*: a
...
```xml
<bpmn:extensionElements>
  <dryrun:binding type="http">
    <dryrun:property name="method" value="POST"/>
    <dryrun:property name="url" value="https://internal.messages-api/v1/messages"/>
  </dryrun:binding>
  <dryrun:mock>payload.messagesApi = { status: 201, messageId: id() };</dryrun:mock>
</bpmn:extensionElements>
```
...
### `dryrun:test` — embedded acceptance test
...
### `dryrun:scenario` — named walkthrough input
...
### `dryrun:collection` — multi-instance data binding
...
<bpmn:multiInstanceLoopCharacteristics isSequential="true">
  ...
  <dryrun:collection expression="participants" elementVariable="participant"/>
</bpmn:multiInstanceLoopCharacteristics>
```

The standard `loopCardinality` stays (rule 1: engines that only need a count
still get it); `dryrun:collection` adds what the simulator and binding pass
need. Without it the sim falls back to cardinality-only (spike behaviour).

### `dryrun:sample` — message/signal payload for simulation
...
<bpmn:message id="Msg_KinesisRecord" name="kinesis-record">
  <bpmn:extensionElements>
    <dryrun:sample correlationKey="chatId">{"partitionKey":"chat_8231","data":{"messageId":"msg_5f2e9c"}}</dryrun:sample>
  </bpmn:extensionElements>
</bpmn:message>
```
...
| Consumer | Standard constructs | `dryrun:mock` | `dryrun:binding` | `dryrun:test` / `dryrun:scenario` | `dryrun:collection` / `dryrun:sample` |
|---|---|---|---|---|---|
...
| `dryrun:` | Camunda 8 | Camunda 7 / Flowable |
|---|---|---|
...
| `dryrun:collection` | `zeebe:LoopCharacteristics inputCollection/inputElement` | `camunda:collection` / `camunda:elementVariable` |
| `dryrun:sample correlationKey` | `zeebe:Subscription correlationKey` | message correlation config |
| `dryrun:mock` | worker stub scaffold (job type from binding) | delegate stub scaffold |

## Moddle schema sketch

```json
{
  "name": "Dryrun", "prefix": "poc",
  "uri": "https://dryrun.dev/schema/1.0",
  "types": [
...
- `dryrun:collection` carries execution-relevant data (like every engine's MI
...
- `dryrun:mock` bodies are JS by definition (browser runtime); a `language`
  attribute is reserved for the future but not v1.
- `dryrun:binding` types are a folksonomy, not a registry — the inventory
