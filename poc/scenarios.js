 ...
 *
 * Plain script, no build: defines window.SCENARIOS.
 * Step shape: { elements, edges?, title, note, logicLabel, logic, apply? }
 */
(function () {
  'use strict';

  const SANITISE_LOGIC = `// security-filters/redact.js — applied to every inbound body field
const FILTERS = [
...
retry: exponential backoff ×5 → dead-letter on exhaustion`;
  ...
  function evaluatePolicy(p, decision, policy, reason) {
    const next = clone(p);
  ...
  function save(p) {
    ...
    next.messagesApi = {
      status: 201,
      messageId: 'msg_5f2e9c',
      persistedAt: '2026-08-29T10:14:03.221Z'
    };
    return next;
  }
  ...
  function kafkaDelivery(p) {
    const next = clone(p);
    next.deliveries = (next.deliveries || []).concat({
      channel: 'kafka',
      recipient: 'usr_2001',
      topic: 'chat-message-deliveries',
      key: 'usr_2001',
  ...
  const deniedPayload = clone(basePayload);
  deniedPayload.request.body.senderId = 'usr_6660';
  deniedPayload.request.body.text = 'hey — card 4111 1111 1111 1111';
